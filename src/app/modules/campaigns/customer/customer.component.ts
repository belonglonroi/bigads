...
export class CustomerComponent extends BaseClass implements OnInit, OnChanges {
    ...
    openDialog(e?, method?) {
        const dialogRef = this.dialogService.open(DialogCustomerComponent, {
            header: method
                ? this.translate.instant('Detail_customer')
                : !e
                ? this.translate.instant('Add_customer')
                : this.translate.instant('Update_customer'),
            width: '400px',
            data: {
                ...e,
                method: method,
    ...
    bulkCreateCustomer(data) {
        ...
        this.customerService
            .bulkCreateCustomer({ customers: [...param] })
            .pipe(this.unsubsribeOnDestroy)
            .subscribe({
                next: (res) => {
                    if (res.data) {
